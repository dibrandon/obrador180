...
> En entorno local se está migrando el acceso admin desde `?k=` a un login con clave persistente y guard (`/admin/login` + `/admin/ping`), como base de la versión v0.8.
...
> El acceso al panel admin está protegido por clave (`ADMIN_KEY` en backend) y, en la nueva versión local, mediante login en `/admin/login`.
...
| 5️⃣    | Dashboard + métricas básicas + Auth     | 🔄 En progreso (v0.8-pre)              |
...
* `docs/resumenDeploy6.md` → Deploy completo Render + Vercel (v0.7)

---

## 🔖 Próxima versión

**Objetivo v0.8 – “Dashboard & Auth refinado”**

* Pulir login admin basado en clave persistente (Basic + storage local) y guard de rutas.
* Dashboard con estadísticas básicas (productos activos, inactivos, total).
* Mejoras visuales y footer informativo.
* Primeras optimizaciones de frontend (bundle + imágenes Cloudinary).
